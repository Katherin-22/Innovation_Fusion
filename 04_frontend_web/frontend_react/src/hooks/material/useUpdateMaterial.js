import { useState } from "react";
import { updateMaterial } from "../../services/administrador/MaterialService";

export const useUpdateMaterial = () => {
  const [load, setLoad] = useState(false);
  const [success, setSuccess] = useState(false);

  const handleUpdateMaterial = async (id, data) => {
    setLoad(true); // paso 1: activar "cargando"
    try {
      await updateMaterial(id, data); // paso 2: enviar datos al backend
      setSuccess(true);        // paso 3: si todo ok → marcar éxito
    } catch (error) {
      console.error("Error al actualizar material:", error);
      setSuccess(false);      // si falla → marcar como no exitoso
    } finally {
      setLoad(false);         // paso 4: quitar "cargando"
    }
  };

  return { handleUpdateMaterial, load, success };
};
